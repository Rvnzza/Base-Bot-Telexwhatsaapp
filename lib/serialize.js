...
const serialize = async (conn, m) => {
  ...
  if (m.key) {
    ...
    m.isOwner = m.sender == m.botNumber || m.sender == ownerNum || own.includes(m.sender);
    if (m.isGroup)
  ...
  const botname = global.botname || "Revinzabotz";
  ...
  m.reply = async (revinza) => {
    if (revinza === null || revinza === undefined) return null;
   
    return conn.sendMessage(m.chat, {
      text: revinza.toString(),
      contextInfo: {
